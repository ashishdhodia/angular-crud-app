...
export class ProductComponent implements OnInit {
  ...
  onBuy(id: any) {
    this.getJson.getDataCartToPost(id).subscribe((data) => {
      this.cartData = { ...data, "quantity": 1 };
      this.cartData = { ...this.cartData, "originalPrice": this.cartData.price };
      // console.log(this.cartData);
      this.getJson.postDataCart(this.cartData).subscribe((data) => {
        console.log(data);
        // window.location.reload()
      });
      let message = this.cartData.product_name
      alert(`Added ${message} to the cart`);
